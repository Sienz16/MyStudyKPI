...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $existingImagePath = "";
    $existingImagePathQuery = mysqli_query($conn, "SELECT img_path FROM competition WHERE comp_id = $id AND userID = {$_SESSION["UID"]}");
    ...
    if (isset($_FILES["fileToUpload"]) && $_FILES["fileToUpload"]["name"] == "") {
        $sql = "UPDATE competition SET sem = $sem, year = '$year', comp_name = '$comp_name', 
                level = '$level', remark = '$remark'
                WHERE comp_id = $id AND userID = " . $_SESSION["UID"];
    
        $status = update_DBTable($conn, $sql);
    
        if ($status) {
            echo "Your data has been updated successfully! :)<br>";
            echo '<a href="competition.php">Go Back</a>';
        } 
        else {
            echo '<a href="competition.php">Go Back</a>';
        }
    ...
    // If there is an image
        ...
        if ($uploadOk){
            $sql = "UPDATE competition SET sem = $sem, year = '$year', comp_name = '$comp_name', 
                level = '$level', remark = '$remark', img_path = '$uploadFileName' 
                WHERE comp_id = $id AND userID = " . $_SESSION["UID"];
